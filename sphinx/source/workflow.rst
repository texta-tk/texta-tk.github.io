...
Datasets can be closed and opened. Closed datasets are not listed to the users. Datasets can also be removed from TEXTA and Elasticsearch.
...
Now we can tag a dataset with our trained tagger via :ref:`Apply Preprocessor <applyPreprocessor>`. All taggers we have trained with Texta tool, can be found under 'Text Tagger preprocessor'.
...
----------------------
In Figure 7.1 we can see the general outlook of Apply Preprocessor. Here we can apply the same :ref:`preprocessors <preprocessors>` we could have applied in the Dataset Importer or apply :ref:`the taggers <classificationmanager>` we have trained ourselves (under ‘Text Tagger preprocessor’). We can use all the documents or a subset gotten via Search. We can select a field and start preprocessing. We can see the results in 'Tasks for: Apply Preprocessor'.

.. figure:: images/11-4_applypreprocessoroutlook.png

	Figure 7.1. *Apply Preprocessor outlook*

	
These tags we can later search for in the :ref:`Searcher <searcher>` (Figure 7.2). We can get the tagged documents via 'fact_text_values' field with FACT_NAME as TEXTA_TAG. In the example below we can choose to search for documents with (*is*) or without (*not*) the tag chosen in the FACT_VAL box 'Value (case sensitive)'.
