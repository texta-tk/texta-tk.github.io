...
This is version 1.6 documentation.
...
.. note::
    ...
    Restricted contains the superfuser tools for managing users, datasets, language models and text classifiers.
...
The biggest bosses in TEXTA Toolkit are the superusers, whose privileges include:
	1. Managing Users and their access rights (User Access in Admin)
	2. Managing and importing datasets (Dataset Management in Admin and Importer)
	3. Training language models (Train Language Model in Task Manager)
	4. Training and applying text classifiers (Train Text Tagger in Task Manager)
	5. Apply preprocessors (Apply Preprocessor in Task Manager)
...
Users and their access to datasets can be configured in the User Access Management panel in Admin:
...
Datasets can be closed and opened. Closed datasets are not listed to the users. 
The size of the dataset can be seen in the table and the doc count means how big this index is. 
Datasets can be deleted from TEXTA and Elasticsearch or only the link between Elasticsearch index and TEXTA can be removed.

Multiple datasets can be added using wildcards. 
...
which can be found under "Importer" menu on the top.

Dataset Importer ("importer" from now on) is a tool which allows to insert data in many formats and 
store it in the underlying Elasticsearch database, so that it could be then used for analyzing using the other
...
All the import jobs that have been completed or are still in progress are displayed on the right panel. Here we
...
with "Train Language Model" application in "Task Manager" (available for superusers only).
...
By default, all exsisting documents in the given dataset are used ('Select a search: Empty (all documents)'). We can also train a model on the data we have filtered out with the :ref:`Searcher <searcher>`.
...
After starting the model training task, we can see the progress. For progress upgrade, we have to refresh the page. Once the training completes, we can see the following on the right panel: 
...
We can set negative multiplier to change ratio of negative examples and choose maximum sample size per class. By changing the Training set score we can choose the relevance of the documents that are taken into count and we can choose by changing the language model which area is being clustered. 

.. figure:: images/11-1_new_model.png

    Figure 6.1. *Choosing parameters for the classification model*
	
Trained models and models in training are shown on the right panel with their training status, parameters, etc. We can delete the model or download it. You can see the changes by refreshing the page.
	
.. figure:: images/11-2_trained_models.png

	Figure 6.2 *Trained models*
	
**Tagging the dataset with the model**

Now we can tag a dataset with our trained tagger via :ref:`Apply Preprocessor <applyPreprocessor>`. All taggers we have trained with Texta tool, can be found under Processor -> 'Text Tagger'.

.. figure:: images/11-3_apply_model.png

    Figure 6.3. *Tagging a dataset with our tag*

After pressing 'Start task', a tagging job will start and it's task status will be shown in the table when completed. We can check the progress by refreshing the page.
	
.. figure:: images/11-4_applied_models.png

    Figure 6.4. *Applied classification models*
...
Entity Extractor lets users manually tag entities and train statistical models to automatically extract them. In order to train a Entity Extractor, we are required to define some mandatory parameters (see Figure 7.1):

	1. A :ref:`search <searcher>` to define the set of documents used to train the model (positive documents). 
	2.	1) The field describes the fact(s), of which the values will be used for training. 
		2) Instead of that or besides that we can also add the lexicon(s) to do the same thing. With this you need to define the field(s) where the lexicon terms will be matched in. 
	3. The name for the class or "tag", which is later user to tag the documents.

.. note::
	In order to train a Entity Extractor we must have :ref:`a search saved in the Searcher <searcher_save>`. 

.. figure:: images/12-1_extractor_param.png

	Figure 7.1 *Model parameters*

Trained models and models in training are shown on the right panel with their training status, parameters, etc. We can delete the model by selecting it or download it from the sign next to it. We can see parameters and results by clicking on the green button. You can see the changes by refreshing the page.

.. figure:: images/12-2_trained_models_entity.png

	Figure 7.2 *Trained Entity Extractor Model*

**Tagging the dataset with the model**

Now we can tag a dataset with our trained tagger via :ref:`Apply Preprocessor <applyPreprocessor>`. All extractors we have trained with Texta tool, can be found under Processor -> 'Entity Extractor'.

.. figure:: images/12-3_apply_model_entity.png

    Figure 7.3. *Extracting dataset with our extractor*

After pressing 'Start task', a extracting job will start and it's task status will be shown in the table when completed. We can check the progress by refreshing the page.
	
.. figure:: images/12-4_applypreprocessoroutlook_entity.png

    Figure 7.4. *Applied classification models*
	
.. note::
	If the dataset contains many documents, the extracting process can be expected to take a few minutes.
	
.. _applyPreprocessor:

Applying Preprocessors
----------------------
In Figure 8.1 we can see the general outlook of Apply Preprocessor. Here we can apply the same :ref:`preprocessors <preprocessors>` we could have applied in the Dataset Importer or apply :ref:`the taggers <classificationmanager>` we have trained ourselves (under Preprocessor -> ‘Text Tagger’). We can use all the documents or a subset gotten via Search. We can select a field and start preprocessing. We can see the results on the right panel.

.. figure:: images/11-4_applypreprocessoroutlook.png

	Figure 8.1. *Apply Preprocessor outlook*

	
These tags we can later search for in the :ref:`Searcher <searcher>` (Figure 8.2).
	
.. figure:: images/07-02_searching_tagged.png

    Figure 8.2. *Searching for tagged documents in the Searcher*
...
Searcher's graphical interface consists of serveral important panels, which are depicted in figure 9.1. You can collapse panels by clicking on the minus in the corner or open by clicking on the plus.
...
In order to add a constraint, we must first choose one or several fields (don't forget to add them by clicking "Add"). After the field is selected, we can then specify which textual tokens should or
...
.. figure:: images/06-00_choosing_a_field.png
    ...
    Figure 9.2. *Choosing a field to filter out*
...
.. figure:: images/06-1_bribe_search_constraints_exact.png
    ...
    Figure 9.3. *"Bribery" and "official" search constraints*

Figure 9.3 shows how we have defined that we want to find all the documents which contain "bribery" *and* "official". We can also choose '*or*' or '*not*' under the Bool. In this case we either get documents containing at least one of the words ('*or*') or definitely not containing the words listed ('*not*').
...
.. figure:: images/06-02-example-search.png
	...
	Figure 9.4. *Example search using Slop*
...
.. figure:: images/06-1_bribe_search_constraints.png
	...
	Figure 9.5. *Searching documents with words starting with 'bribe'*
...
.. figure:: images/06-05_choose_date_range.png
	...
	Figure 9.6. *Choosing a date range from the 1st of May 2010 to the 31st of August 2010*

If we are working with long documents, we can also choose to look up short version of the text in the Search Options as shown in Figure 9.7. In the figure the window size selected is 3 words. That means we can see the word and 3 words before and after the word we are looking for. In our example search we will leave this unticked.
As shown on figure 9.7 we can disable html stripping. 

.. figure:: images/06-5_choosing_search_context_range.png

	Figure 9.7. *Choosing context window size*

If we click on "Search" button, we will see the matching data in a tabular form (see Figure 9.9), where layered features share feature name's prefix, and
...
If there are too many features (columns), we can hide or show them from the drop-down menu in the upper left corner. We can select or deselect all of them together or by clicking on them saparately.

.. figure:: images/06-2_bribe_results.png

    Figure 9.9. *Bribe search results*

	1. Show or hide features
.. _searcher_save:

After we have come up with a suitable search, we can save it for later uses (see figure 9.10).

 
.. figure:: images/06-7_saving_a_search.png

	Figure 9.10. *Searcher's save and export icon*
	
	1. Export/Delete results
	2. Save icon
	3. View query
	4. View on full screen
...
We can accept or reject a suggestion. If we accept, the document's ID will appear in the 'Selected documents'.
...
.. figure:: images/06-07_cluster_searcher.png
	...
	Figure 9.11. *Cluster Searcher's first look*


.. figure:: images/06-10_example_result_of_cluster_search.png

	Figure 9.12. *Example result of Cluster Search without a stopword lexicon*
...
constraints to retrieve the data and then use query result actions (see Figure 9.13), such as *export*. 
Export panel button is shown on the figure 9.10.

.. figure:: images/06-3_export_panel.png

    Figure 9.13. *Export panel*

Export panel allows to specify, how many rows and which features are we interested in. Exported data is in CSV format. If we select 'Selected features', only shown features are exported (hidden features are not exported).
...
The second action on search results (see Figure 9.10.) is deletion - if we detect some malformed data or are simply not interested in some subset, we can remove it
permanently from the Elasticsearch.
    
Using saved searches
++++++++++++++++++++

Searches can be saved with clicking on the save icon next to Current Search title (see Figure 9.10). If we save our "bribery" search under "bribery", we can see it being listed in "Saved Searches" panel.

.. figure:: images/06-4_saved_search.png

    Figure 9.14. *Saved searches*
...
"bribe" search. By requesting aggregation as shown on Figure 9.15, we get the result on the same figure.

.. figure:: images/06-5_simple_aggregation.png

    Figure 9.15. *Simple aggregation*

From the results in Figure 9.15 we can see raw word distributions for both checked "bribe" search and "Current Search" (which doesn't have any constraints,
...
.. figure:: images/06-13_visualizing_facts.png
    ...
    Figure 9.16. *Fact Graph*
    
.. figure:: images/06-14_visualizing_facts_results.png

    Figure 9.17. *Fact Graph Results*
...
Here we can get an overview of our lexicons created and concepts commited. In Figure 10.1 we can already see some concepts. The creation of these concepts are described below. We can delete the concepts by clicking on the little minus sign and then confirming our decision by choosing *Yes* in the question box shown in Figure 10.2.

.. figure:: images/09-01_terminology_overview.png

	Figure 10.1. *Terminology Overview*

.. figure:: images/09-02_remove_entry.png

	Figure 10.2. *Removing concept*
...
The first batch of suggested words are shown in figure 11.3. The result would be better if we had trained our model on lemmas. This one is trained on text field, therefore we get all forms of 'accuse' in the candidates list.

.. figure:: images/07-2_first_suggestion_batch.png

    Figure 11.3. *First suggestion batch*
...
"Terminologies". It takes lexicons as input and outputs concepts, which user defines using the graphical tool. Words are displayed on
...
After checking the lexicons and plotting them, we get to the state depicted in figure 12.1.

.. figure:: images/08_conceptualiser_initial_plot.png

    Figure 12.1. *Conceptualiser's initial plot*
...
.. figure:: images/	08-03_grouping_concepts.png
	...
	Figure 12.2. *Grouping concepts and naming a concept*
	
	
.. figure:: images/08-1_grouped_concepts.png
    
    Figure 12.3. *After grouping the words into concepts*
...
Mining task requires parameters - much like training language models. In Figure 13.1 we can see the parameters we can use.

.. figure:: images/09_mwe_parameters.png

    Figure 13.1. *Multi-word expression mining parameters*
...
.. figure:: images/09-1_mwe_progress.png
    ...
    Figure 13.1. *Multi-word expression task progress*
...
.. figure:: images/09-2_mwe_results.png
    ...
    Figure 13.2. *Multi-word expression results*
